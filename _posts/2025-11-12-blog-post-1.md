...
```
通过`llamafactory-cli version`检查是否安装成功，大概率会出现**环境冲突**，使用`pip install --no-deps -e .`解决，自己安装相应的版本依赖
```
conda create -n llamafactory python=3.1.0
conda activate llamafactory
pip install --no-deps -e .
pip install torch transformers omegaconf datasets==4.0.0 peft==0.17.1 trl==0.9.6 gradio
```
...
## WebUI
通过`llamafactory-cli webui`指令进入WebUI, `curl cip.cc`查询服务器IP公网地址，访问http://ip:port进入webUI
